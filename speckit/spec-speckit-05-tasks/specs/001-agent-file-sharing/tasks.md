...
- [ ] T001 Install required dependencies: react-markdown, remark-gfm, zod via pnpm
- [ ] T002 [P] Initialize shadcn/ui with `npx shadcn@latest init`
- [ ] T003 [P] Add shadcn/ui components: button, card, input, textarea, badge
- [ ] T004 [P] Create component directory structure: src/components/client/ and src/components/server/
- [ ] T005 [P] Create server directory structure: src/server/data/ and src/server/actions/
- [ ] T006 [P] Create lib directory structure: src/lib/utils/ and src/lib/validation/
- [ ] T007 [P] Create types directory: src/types/
- [ ] T008 Configure TypeScript strict mode if not already enabled in tsconfig.json
...
- [ ] T009 [P] Create AgentFile type interface in src/types/AgentFile.ts
- [ ] T010 [P] Create Pagination types (PaginationState, PaginatedResponse) in src/types/Pagination.ts
- [ ] T011 [P] Create Clipboard types (CopyResult) in src/types/Clipboard.ts
- [ ] T012 [P] Create validation schema with Zod for agent file form in src/lib/validation/agentFileSchema.ts
- [ ] T013 [P] Create app constants (PAGE_SIZE, max lengths) in src/lib/constants.ts
- [ ] T014 Create mock agent files data store with initial dataset (5 files) in src/server/data/mockAgentFiles.ts
- [ ] T015 Implement helper functions (getAgentFiles, getAgentFileById, addAgentFile) in src/server/data/mockAgentFiles.ts
- [ ] T016 [P] Create clipboard utility function (copyToClipboard) in src/lib/utils/clipboard.ts
- [ ] T017 [P] Create pagination utility function (calculatePagination) in src/lib/utils/pagination.ts
- [ ] T018 Create Server Actions file with getAgentFiles, getAgentFileById, postAgentFile in src/server/actions/agentFileActions.ts
- [ ] T019 Add error boundary component in app/error.tsx
- [ ] T020 Update root layout metadata for SEO in app/layout.tsx
...
- [ ] T021 [P] [US1] Create AgentFileCard Server Component to display individual file preview in src/components/server/AgentFileCard.tsx
- [ ] T022 [P] [US1] Create EmptyState Server Component for when no files exist in src/components/server/EmptyState.tsx
- [ ] T023 [US1] Create AgentFileList Server Component to render grid of file cards in src/components/server/AgentFileList.tsx
- [ ] T024 [US1] Create PaginationControls Client Component with next/prev buttons in src/components/client/PaginationControls.tsx
- [ ] T025 [US1] Implement home page (app/page.tsx) with pagination using searchParams, fetching files, and rendering list
- [ ] T026 [US1] Add navigation link to post page in home page layout
- [ ] T027 [US1] Test pagination with >12 mock files by adding more data to mockAgentFiles.ts
- [ ] T028 [US1] Verify empty state displays when no files available
- [ ] T029 [US1] Verify pagination controls disable at boundaries (first/last page)
